...
- **Measured with live agents** - agents building a WordPress plugin screen with the CLI used the design system's current components from the right package in 30 of 30 choices. Without it, from memory, they did so in 2 of 15, and two of three files imported names the packages don't export; with the packages installed they managed 11 of 15 at about twice the tool calls and time (3-6 runs per condition, one model; see the live agent tests in [`eval/LOG.md`](eval/LOG.md)).
