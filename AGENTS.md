...
- `test/fixtures/primer/` holds real Primer data (the Storybook manifest from https://primer.style/react/storybook and `@primer/react@38.39.0` metadata), and `test/fixtures/wordpress/` a six-entry slice of Gutenberg's manifest covering compound components (`Tabs.Root`) and multi-statement imports. Counts and names asserted in tests are pinned to the committed files; `scripts/refresh-fixtures.sh` recreates both, and a refresh updates those expectations in the same change.
