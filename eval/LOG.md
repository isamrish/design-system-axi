...
## 2026-09-17 — Live agents on Primer: the CLI against no CLI

The Gutenberg comparison above found an effect of the CLI with packages installed. This checks whether it holds on a design system the model knows well. Same design as that controlled pair: 3 Claude Sonnet agents per arm, each in its own clone of a Vite + React + TypeScript app with `@primer/react` 38.39.0 installed, required to pass `tsc`; the CLI arm adds the released 0.1.6 CLI, a live-synced catalog, its session hook, and a logging wrapper. The task has a trap in every item: deprecated components that still ship (`Flash`, `Pagehead`, `TabNav`, the old `UnderlineNav`) or `@primer/react/experimental` import paths (`Blankslate`, `DataTable`). Expected components were recorded before any run. Data, method, and a scorer are in [`experiments/2026-09-17-live-agents/primer-cli-comparison/`](experiments/2026-09-17-live-agents/primer-cli-comparison/).

| arm (3 runs each)          | current component, right path | deprecated components used | files that type-check | tool calls: median (range) | tokens (mean) | time (median) |
| -------------------------- | ----------------------------- | -------------------------- | --------------------- | -------------------------- | ------------- | ------------- |
| No CLI, packages installed | 15/15                         | none                       | 3 of 3                | 34 (29-39)                 | 108k          | 3.5 min       |
| CLI, packages installed    | 15/15                         | none                       | 3 of 3                | 30 (21-33)                 | 120k          | 3.2 min       |

No measurable difference: every run in both arms avoided every trap and type-checked, with similar effort; the CLI runs used more tokens on average, within the spread of three runs. Two likely reasons, both specific to Primer: the model knows it well, and its installed package describes itself. The no-CLI agents reported reading `generated/components.json`, the curated metadata design-system-axi also syncs (two runs), and the `@deprecated` notes in its type declarations (one run). `@wordpress/ui` has neither a well-known history nor such an index, which is where the CLI made choices consistent (15/15 against 11/15).

So the measured value of the CLI is conditional: it helped where the design system's published documentation says more than the model remembers or the installed package exposes, and it added nothing measurable for a well-known design system whose package ships its own metadata. Limits: one model, one task per design system, 3 runs per arm; the task centred on choosing components, and a task that depends on less discoverable props or usage examples could behave differently.
